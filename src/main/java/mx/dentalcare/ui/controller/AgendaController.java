...
public class AgendaController {
    ...
    private void configurarEventos() {
        ...
        btnAnterior.setOnAction(event -> {
            ...
            String vista =
                    cmbVista.getSelectionModel().getSelectedItem();
    ...
    private void configurarRelojHoraActual() {
        ...
        relojHoraActual = new Timeline(
                new KeyFrame(
                        Duration.seconds(1),
                        event -> actualizarIndicadorHoraActual()
                )
        );
    ...
    private void actualizarIndicadorHoraActual() {
        ...
        if ("Día".equals(vista)) {
            ...
            LocalDate inicioSemana =
                    obtenerInicioSemana(fechaActual);

            LocalDate finSemana =
                    inicioSemana.plusDays(6);

            mostrar =
                    !hoy.isBefore(inicioSemana) &&
                            !hoy.isAfter(finSemana);
        }

        LocalTime inicio =
                LocalTime.of(HORA_INICIO, 0);

        LocalTime fin =
                LocalTime.of(HORA_FIN, 0);
        ...
        double minutosDesdeInicio =
                (ahora.getHour() - HORA_INICIO) * 60
                        + ahora.getMinute()
    ...
    private void configurarRedimensionamiento() {
        ...
        scrollAgenda.viewportBoundsProperty()
                .addListener((observable, anterior, actual)
                        -> ajustarAnchoAgenda());

        scrollAgenda.widthProperty()
                .addListener((observable, anterior, actual)
                        -> ajustarAnchoAgenda());

        contenedorAgenda.widthProperty()
                .addListener((observable, anterior, actual)
                        -> ajustarAnchoAgenda());
    }
    ...
    private void ajustarAnchoAgenda() {
        ...
        double anchoViewport =
                scrollAgenda.getViewportBounds().getWidth();
        ...
        int numeroDias =
                "Día".equals(vista)
                        ? NUMERO_DIAS_VISTA_DIA
                        : NUMERO_DIAS;

        double anchoMinimoDias =
                ANCHO_DIA_MINIMO * numeroDias;

        double anchoDisponibleDias =
                anchoViewport - ANCHO_HORA;
        ...
        if (anchoDisponibleDias >= anchoMinimoDias) {
            ...
            anchoDia =
                    anchoDisponibleDias / numeroDias;
        ...
        if (contenedorAgenda.getChildren().get(0)
                instanceof HBox) {

            HBox encabezado =
                    (HBox) contenedorAgenda.getChildren().get(0);

            encabezado.setPrefWidth(anchoTotal);
            encabezado.setMinWidth(anchoTotal);

            if (!encabezado.getChildren().isEmpty()
                    && encabezado.getChildren().get(0)
                    instanceof VBox) {

                VBox espacioHora =
                        (VBox) encabezado.getChildren().get(0);
            ...
            for (int i = 1;
                 i < encabezado.getChildren().size();
                 i++) {

                if (encabezado.getChildren().get(i)
                        instanceof VBox) {

                    VBox dia =
                            (VBox) encabezado.getChildren().get(i);
        ...
        if (contenedorAgenda.getChildren().get(1)
                instanceof HBox) {

            HBox cuerpo =
                    (HBox) contenedorAgenda.getChildren().get(1);

            cuerpo.setPrefWidth(anchoTotal);
            cuerpo.setMinWidth(anchoTotal);

            if (!cuerpo.getChildren().isEmpty()
                    && cuerpo.getChildren().get(0)
                    instanceof VBox) {

                VBox columnaHoras =
                        (VBox) cuerpo.getChildren().get(0);
            ...
            for (int i = 1;
                 i < cuerpo.getChildren().size();
                 i++) {

                if (cuerpo.getChildren().get(i)
                        instanceof AnchorPane) {

                    AnchorPane columnaDia =
                            (AnchorPane) cuerpo.getChildren().get(i);
        ...
        scrollAgenda.setFitToWidth(
                anchoTotal <= anchoViewport + 1
        );
    }

    private void establecerAncho(
    ...
    private void actualizarAgendaSemana() {
        ...
        List<Cita> citas =
                citaService.obtenerPorRango(
                        inicioSemana.atStartOfDay(),
                        inicioSemana.plusDays(7).atStartOfDay()
                );
    ...
    private void construirEncabezadoDiaVista(
            LocalDate fecha) {

        HBox encabezado = new HBox();

        encabezado.getStyleClass()
                .add("agenda-week-header");
        ...
        espacioHora.getStyleClass()
                .add("agenda-time-header");

        encabezado.getChildren()
                .add(espacioHora);

        VBox dia =
                construirEncabezadoDia(fecha);

        encabezado.getChildren().add(dia);

        contenedorAgenda.getChildren()
                .add(encabezado);
    }

    private void actualizarRangoFechaDia(
            LocalDate fecha) {
    ...
    private void construirCuerpoDia(
            LocalDate fecha,
            List<Cita> citas) {

        HBox cuerpo = new HBox();

        cuerpo.getStyleClass()
                .add("agenda-week-body");

        VBox columnaHoras =
                construirColumnaHoras();

        cuerpo.getChildren()
                .add(columnaHoras);

        AnchorPane columnaDia =
                construirColumnaDia(fecha, citas);
    ...
    private LocalDate obtenerInicioSemana(
            LocalDate fecha) {
    ...
    private void actualizarRangoFecha(
            LocalDate inicio,
            LocalDate fin) {
        ...
        lblRangoFecha.setText(
                textoInicio + " - " + textoFin
        );
    }

    private void construirEncabezadoSemana(
            LocalDate inicioSemana) {
    ...
    private VBox construirEncabezadoDia(
            LocalDate fecha) {
        ...
        dia.getStyleClass()
                .add("agenda-day-header");

        if (fecha.equals(LocalDate.now())) {

            dia.getStyleClass()
                    .add("agenda-day-header-today");
        }
        ...
        numeroDia.getStyleClass()
                .add("agenda-day-number");
    ...
    private void construirCuerpoSemana(
            LocalDate inicioSemana,
            List<Cita> citas) {
    ...
    private VBox construirColumnaHoras() {
        ...
        columnaHoras.getStyleClass()
                .add("agenda-time-column");

        for (int hora = HORA_INICIO;
             hora < HORA_FIN;
             hora++) {
            ...
            etiqueta.setPrefHeight(
                    ALTURA_POR_HORA
            );

            etiqueta.setMinHeight(
                    ALTURA_POR_HORA
            );

            etiqueta.setMaxHeight(
                    ALTURA_POR_HORA
            );
            ...
            etiqueta.getStyleClass()
                    .add("agenda-time-label");

            columnaHoras.getChildren()
                    .add(etiqueta);
        }
    ...
    private AnchorPane construirColumnaDia(
            LocalDate fecha,
            List<Cita> citas) {
        ...
        columna.getStyleClass()
                .add("agenda-day-column");

        construirLineasHorarias(
                columna,
                alturaTotal
        );

        columna.setOnMouseClicked(event -> {
    ...
    private LocalTime calcularHoraDesdeY(
            double y) {
    ...
    private void agregarIndicadorHoraActual(
            AnchorPane columna) {

        Pane indicador = new Pane();

        indicador.getStyleClass()
                .add("agenda-current-time-line");
    ...
    private void construirLineasHorarias(
            AnchorPane columna,
            double alturaTotal) {
        ...
        for (int hora = HORA_INICIO;
            ...
            lineaMedia.getStyleClass()
                    .add("agenda-half-hour-line");
    ...
    private void agregarCita(
        ...
        LocalTime horaInicio =
                cita.getInicio().toLocalTime();

        double minutosDesdeInicio =
                (horaInicio.getHour() - HORA_INICIO)
                        * 60
    ...
    private VBox crearBloqueCita(Cita cita) {
        ...
        if (cita.getEstado() != null) {
            ...
            switch (cita.getEstado()) {
                ...
                case PROGRAMADA:
                    bloque.getStyleClass()
                            .add("agenda-event-programada");
                    break;

                case CONFIRMADA:
                    bloque.getStyleClass()
                            .add("agenda-event-confirmada");
                    break;

                case ATENDIDA:
                    bloque.getStyleClass()
                            .add("agenda-event-atendida");
                    break;

                case CANCELADA:
                    bloque.getStyleClass()
                            .add("agenda-event-cancelada");
                    break;

                case NO_ASISTIO:
                    bloque.getStyleClass()
                            .add("agenda-event-no-asistio");
                    break;
            }
        }
        ...
        paciente.setWrapText(false);
        paciente.setMaxWidth(Double.MAX_VALUE);

        paciente.getStyleClass()
                .add("agenda-event-patient");
        ...
        Label tratamiento =
                new Label(
                        obtenerResumenTratamientos(cita)
                );

        tratamiento.setWrapText(false);
        tratamiento.setMaxWidth(Double.MAX_VALUE);

        tratamiento.getStyleClass()
                .add("agenda-event-treatment");
        ...
        horario.getStyleClass()
                .add("agenda-event-time");
        ...
        /*
         * El motivo y las notas ya no forman
         * parte visual de la tarjeta.
         *
         * Se muestran mediante Tooltip.
         */
        instalarTooltipCita(
                bloque,
                cita
        ...
        bloque.setOnContextMenuRequested(event -> {
            ...
            Label encabezadoEstado =
                    new Label(
                            "Estado de la cita"
                    );

            encabezadoEstado.getStyleClass()
                    .add("context-menu-section-label");

            CustomMenuItem itemEncabezadoEstado =
                    new CustomMenuItem(
                            encabezadoEstado
                    );

            itemEncabezadoEstado
                    .setHideOnClick(false);

            menu.getItems()
                    .add(itemEncabezadoEstado);
    ...
    /**
     * Devuelve el resumen que aparecerá en la tarjeta
     * de la agenda.
     *
     * Ejemplos:
     *
     ...
     */
    private String obtenerResumenTratamientos(
            Cita cita) {

        if (cita.getTratamientos() == null
                || cita.getTratamientos().isEmpty()) {
    ...
    /**
     * Construye el contenido completo del Tooltip
     * de una cita.
     */
    private String construirTooltipCita(
            Cita cita) {
        ...
        if (cita.getTratamientos() != null
            ...
            if (cita.getTratamientos().size() > 1) {
                detalle.append("s");
            }

            detalle.append(":\n");

            for (TratamientoAplicado tratamiento
                    : cita.getTratamientos()) {
    ...
    /**
     * Convierte el estado interno de la cita
     * a un texto amigable para el usuario.
     */
    ...
    /**
     * Instala el Tooltip sobre cualquier nodo
     * que represente una cita.
     */
    private void instalarTooltipCita(
            javafx.scene.Node nodo,
            Cita cita) {

        Tooltip tooltip =
                new Tooltip(
                        construirTooltipCita(cita)
                );
    ...
    private void iniciarResize(
        ...
        alturaOriginal =
                bloque.getPrefHeight();

        alturaInicialResize =
                event.getSceneY();

        inicioOriginalResize =
                cita.getInicio();
        ...
        if (!bloque.getStyleClass()
                .contains("agenda-event-resizing")) {

            bloque.getStyleClass()
                    .add("agenda-event-resizing");
        }
    }

    private void actualizarResize(
            MouseEvent event) {
        ...
        resizeValido =
                !existeConflictoDuranteResize(
                        inicioOriginalResize,
                        nuevoFin,
                        citaRedimensionada
                );

        bloqueRedimensionado.setPrefHeight(
                alturaFinal
        );

        bloqueRedimensionado.setMinHeight(
                alturaFinal
        );

        bloqueRedimensionado.setMaxHeight(
                alturaFinal
        );

        actualizarEstiloResize(
                resizeValido
        );
    }

    private LocalDateTime calcularFinDesdeAltura(
            int bloques) {
        ...
        LocalDateTime nuevoFin =
                inicioOriginalResize.plusMinutes(
                        minutos
                );
    ...
    private boolean existeConflictoDuranteResize(
            LocalDateTime nuevoInicio,
            LocalDateTime nuevoFin,
            Cita citaRedimensionada) {
        ...
        for (Cita otra : citas) {
            ...
            if (otra.getId() != null
                    && citaRedimensionada.getId() != null
                    && otra.getId().equals(
                    citaRedimensionada.getId()
            )) {
    ...
    private void actualizarEstiloResize(
            boolean valido) {
        ...
        bloqueRedimensionado.getStyleClass()
                .remove(
                        "agenda-event-resize-valid"
                );

        bloqueRedimensionado.getStyleClass()
                .remove(
                        "agenda-event-resize-conflict"
                );
    ...
    private void finalizarResize() {
        ...
        try {
            ...
            mostrarError(
                    "No se pudo cambiar la duración.",
                    ex.getMessage()
            );
    ...
    private void limpiarEstadoResize() {
        ...
        if (bloqueRedimensionado != null) {
            ...
            bloqueRedimensionado.setCursor(
                    Cursor.HAND
            );
    ...
    private void configurarDragAndDrop(
        ...
        bloque.setOnMouseDragged(event -> {
            ...
            if (!arrastrandoCita) {
                ...
                if (distanciaX < UMBRAL_ARRASTRE
                        && distanciaY < UMBRAL_ARRASTRE) {
    ...
    private void iniciarArrastre() {
        ...
        if (!citaArrastrada.getStyleClass()
                .contains("agenda-event-dragging")) {

            citaArrastrada.getStyleClass()
                    .add(
    ...
    private void actualizarPosicionArrastre(
            MouseEvent event) {
        ...
        double duracion =
                java.time.Duration.between(
                        citaEnArrastre.getInicio(),
                        citaEnArrastre.getFin()
                ).toMinutes();
        ...
        LocalTime nuevaHora =
                calcularHoraDesdePosicion(
                        posicionY,
                        duracion
                );

        LocalDate nuevaFecha =
                obtenerFechaDeColumna(
                        columna
                );
        ...
        LocalDateTime nuevoFin =
                nuevoInicio.plusMinutes(
                        (long) duracion
                );

        boolean valido =
                !existeConflictoDuranteArrastre(
                        nuevoInicio,
                        nuevoFin,
                        citaEnArrastre
                );

        fechaPropuestaArrastre =
                nuevaFecha;

        horaPropuestaArrastre =
                nuevaHora;
        ...
        double nuevaPosicion =
                calcularPosicionDesdeHora(
                        nuevaHora
                );

        if (citaArrastrada.getParent()
                != columna) {
            ...
            if (citaArrastrada.getParent()
                ...
                ((AnchorPane)
                        citaArrastrada.getParent())
                        .getChildren()
                        .remove(citaArrastrada);
            }
        ...
        actualizarEstiloArrastre(
                arrastreValido
        );
    }

    private LocalTime calcularHoraDesdePosicion(
            double posicionY,
            double duracionMinutos) {
    ...
    private double calcularPosicionDesdeHora(
            LocalTime hora) {

        double minutosDesdeInicio =
                (hora.getHour() - HORA_INICIO)
                        * 60.0
                        + hora.getMinute();
    ...
    private AnchorPane obtenerColumnaDesdeEscena(
            double sceneX,
            double sceneY) {

        if (contenedorAgenda.getChildren().size()
                < 2) {
        ...
        for (int i = 1;
            ...
            AnchorPane columna =
                    (AnchorPane)
                            cuerpo.getChildren().get(i);

            Bounds bounds =
                    columna.localToScene(
                            columna.getBoundsInLocal()
                    );
    ...
    private LocalDate obtenerFechaDeColumna(
            AnchorPane columna) {
        ...
        String vista =
                cmbVista.getSelectionModel()
                        .getSelectedItem();

        if ("Día".equals(vista)) {

            if (cuerpo.getChildren().size() > 1
                    && cuerpo.getChildren().get(1)
                    == columna) {
    ...
    private boolean existeConflictoDuranteArrastre(
            LocalDateTime nuevoInicio,
            LocalDateTime nuevoFin,
            Cita citaMovida) {
    ...
    private void actualizarEstiloArrastre(
            boolean valido) {
        ...
        citaArrastrada.getStyleClass()
                .remove(
                        "agenda-event-drag-valid"
                );

        citaArrastrada.getStyleClass()
                .remove(
                        "agenda-event-drag-conflict"
                );
    ...
    private void finalizarArrastre() {
        ...
        try {
            ...
            citaService.cambiarInicio(
                    citaEnArrastre.getId(),
                    nuevoInicio
            );
    ...
    private void abrirNuevaCita(
        ...
        try {
            ...
            NuevaCitaController controller =
                    loader.getController();

            controller.prepararNuevaCita(
                    fecha,
                    hora
            );
    ...
    private void abrirDetalleCita(
        ...
        try {
            ...
            FXMLLoader loader =
                    new FXMLLoader(
                            getClass().getResource(
                                    "/ui/fxml/DetalleCitaDialog.fxml"
                            )
                    );
            ...
            DetalleCitaController controller =
                    loader.getController();
    ...
    private void configurarBusqueda() {
        ...
        txtBuscarPaciente.textProperty()
                .addListener(
    ...
    private void actualizarAgendaMes() {
        ...
        LocalDate ultimoDiaMes =
                fechaActual.withDayOfMonth(
                        fechaActual.lengthOfMonth()
                );

        actualizarRangoFechaMes(
                fechaActual
        );
    ...
    private void actualizarRangoFechaMes(
            LocalDate fecha) {
    ...
    private void construirCalendarioMes(
        ...
        calendario.getStyleClass()
                .add("agenda-month-calendar");

        calendario.setFillWidth(true);

        construirEncabezadoMes(calendario);

        LocalDate primerDiaCalendario =
                primerDiaMes.with(
                        TemporalAdjusters.previousOrSame(
                                DayOfWeek.MONDAY
                        )
    ...
    private void construirEncabezadoMes(
            VBox calendario) {
    ...
    private HBox construirSemanaMes(
            LocalDate inicioSemana,
            List<Cita> citas) {
        ...
        semana.getStyleClass()
                .add("agenda-month-week");
    ...
    private VBox construirCeldaMes(
            LocalDate fecha,
            List<Cita> citas) {
        ...
        numero.getStyleClass()
                .add(
                        "agenda-month-day-number"
                );

        encabezadoDia.getChildren()
                .add(numero);
        ...
        if (citasRestantes > 0) {
            ...
            masCitas.setMaxWidth(
                    Double.MAX_VALUE
            );

            masCitas.getStyleClass()
                    .add(
                            "agenda-month-more"
                    );

            masCitas.setOnMouseClicked(
                    event -> {
    ...
    private void agregarCitaMes(
        ...
        /*
         * Tooltip también en la vista mensual.
         */
        instalarTooltipCita(
                evento,
                cita
    ...
    private void configurarMenuContextualMes(
            Label evento,
            Cita cita) {

        evento.setOnContextMenuRequested(
                event -> {
