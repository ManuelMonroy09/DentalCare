...
public class LoginController {
    ...
    @FXML private void iniciarSesion() {
        ...
        try { authenticationService.login(username, password); mostrarTransicion(); }
    ...
    @FXML private void cerrarVentana() { Stage stage = (Stage) authRoot.getScene().getWindow(); stage.close(); }

    private void mostrarTransicion() {
        txtUsername.setDisable(true); txtPassword.setDisable(true);
        StackPane overlay = new StackPane(); overlay.getStyleClass().add("auth-transition-overlay"); overlay.setOpacity(0);
        StackPane contenido = new StackPane(); contenido.getStyleClass().add("auth-transition-content");
        ProgressIndicator indicador = new ProgressIndicator(); indicador.setProgress(-1); indicador.getStyleClass().add("auth-progress");
        Label titulo = new Label("DentalCare"); titulo.getStyleClass().add("auth-transition-title");
        Label mensaje = new Label("Preparando tu espacio de trabajo..."); mensaje.getStyleClass().add("auth-transition-message");
        javafx.scene.layout.VBox textos = new javafx.scene.layout.VBox(6, titulo, mensaje); textos.setAlignment(javafx.geometry.Pos.CENTER);
        javafx.scene.layout.VBox grupo = new javafx.scene.layout.VBox(18, indicador, textos); grupo.setAlignment(javafx.geometry.Pos.CENTER);
        contenido.getChildren().add(grupo); overlay.getChildren().add(contenido); authRoot.getChildren().add(overlay);
        FadeTransition entrada = new FadeTransition(Duration.millis(220), overlay); entrada.setFromValue(0); entrada.setToValue(1); entrada.play();
        PauseTransition pausa = new PauseTransition(Duration.millis(420));
        pausa.setOnFinished(event -> { try { abrirAplicacion(); } catch (Exception e) { authRoot.getChildren().remove(overlay); txtUsername.setDisable(false); txtPassword.setDisable(false); mostrarError("No fue posible abrir DentalCare. Verifica la configuración de seguridad."); } });
        pausa.play();
    }
    ...
    private void abrirAplicacion() throws Exception {
        ...
        applicationStage.setMinWidth(1100); applicationStage.setMinHeight(700); applicationStage.setResizable(true); applicationStage.show(); applicationStage.centerOnScreen(); loginStage.close();
        root.setOpacity(0); FadeTransition salida = new FadeTransition(Duration.millis(260), root); salida.setFromValue(0); salida.setToValue(1); salida.play();
    }
