...
public class CitaService {
    ...
    public Cita crear(Paciente paciente, LocalDateTime inicio) {
        return crear(paciente, inicio, DURACION_POR_DEFECTO_MINUTOS);
    }
    ...
    private void sincronizarTratamientos(Cita cita, List<Long> tratamientoIds) {
        ...
        for (Long tratamientoId : seleccionados) {
            ...
            TratamientoAplicado tratamientoAplicado = new TratamientoAplicado(
                    tratamiento.getId(), tratamiento.getNombre(), tratamiento.getPrecio(), tratamiento.getDuracionMinutos());
    ...
    public Cita agregarTratamiento(Long citaId, Long tratamientoId) {
        ...
        cita.agregarTratamiento(new TratamientoAplicado(
                tratamiento.getId(), tratamiento.getNombre(), tratamiento.getPrecio(), tratamiento.getDuracionMinutos()));
        return citaRepository.save(cita);
    }
    ...
    public List<Cita> obtenerPorPaciente(Long pacienteId) {
        ...
        return citaRepository.findAll().stream()
                .filter(cita -> cita.getPaciente() != null && cita.getPaciente().getId() != null
    ...
    public List<Cita> obtenerHistorial() {
        return obtenerTodas().stream()
                ...
                .sorted(Comparator.comparing(Cita::getInicio, Comparator.nullsLast(Comparator.reverseOrder())))
                .collect(Collectors.toList());
    }
    ...
    private void publicarCambioEstado(Cita cita) {
        if (cita != null && (cita.getEstado() == EstadoCita.CONFIRMADA || cita.getEstado() == EstadoCita.ATENDIDA)) {
            eventPublisher.publishEvent(new CitaEstadoCambiadoEvent(cita));
        }
    }
    ...
    private void validarInicio(LocalDateTime inicio) {
        if (inicio == null) throw new IllegalArgumentException("La fecha y hora de inicio son obligatorias.");
    }

    private void validarDuracion(long duracionMinutos) {
        if (duracionMinutos <= 0) throw new IllegalArgumentException("La duración de la cita debe ser mayor a 0 minutos.");
    }
    ...
    private Cita obtenerExistente(Long id) {
        if (id == null) throw new IllegalArgumentException("El identificador de la cita no puede ser nulo.");
        return citaRepository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("No existe una cita con el identificador: " + id));
    }
}
